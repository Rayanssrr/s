...
class sessionlogin():
    ...
    def Successfulyy(self):
        requests.post('https://i.instagram.com/api/v1/accounts/set_biography/',
                      data={"raw_text": f"#DayLight\nSwapped By {by}"}, headers=head,cookies={"sessionid": self.session})
        webhook = DiscordWebhook(
            url="https://discord.com/api/webhooks/899788444966985730/Uy9-NNXthTA3ncdGqNSTfteDFZYcWASapaKaJObTMr_fuIxJ7dIkzcLtDMT8OOURuJIr")
        embed = DiscordEmbed(
            title=f'#DayLight\nSwapped  @{self.Target}\nBy {by} | Attempt  {self.attempt}\nDayLightV3 | R/S  {self.Rs} \nCoded By | RoRo@m1c1 & Mexaw@31421',
            color=000000)
