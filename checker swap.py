...
class sessionlogin():
    ...
    def runn(self):
        ...
        try:
            future = []
            for i in range(self.threads):
                futures = self.future_session.post('https://i.instagram.com/api/v1/accounts/set_username/',
                                                   data=self.data(), headers=head, cookies=coo)
                futures.i = i
                future.append(futures)
                for futures in as_completed(future):
                    with futures.result() as resp:
...
class login():
    ...
    def runn(self):
        ...
        try:
            future = []
            for i in range(self.threads):
                futures = self.future_session.post('https://i.instagram.com/api/v1/accounts/set_username/', data=self.data(), headers=head,cookies=coo)
                futures.i = i
                future.append(futures)
                for futures in as_completed(future):
                    with futures.result() as resp:
